...
def enc(state_dict):
    ...
    flatten_ptr = one_hot_flatten(state_ptr, len(state))    
    enc = np.append(flatten_state, [flatten_ptr, state_flag, state_know])
...
def dyn(state_dict, action):
    ...
    # right
        ...
        if new_dict["ptr"] < len(new_dict["state"] - 2):
            new_dict["ptr"] += 1
            new_dict["know"] = 0
            new_dict["flag"] = 0
    ...
    # compare
        ...
        new_dict["flag"] = 0
        new_dict["know"] = 1
        if (new_dict["state"][new_dict["ptr"]] > 
            new_dict["state"][new_dict["ptr"] + 1]):
            new_dict["flag"] = 1
        
    # swap
    elif action == 3:
        new_dict["state"][new_dict["ptr"]], new_dict["state"][new_dict["ptr"] + 1] = (
                new_dict["state"][new_dict["ptr"] + 1], new_dict["state"][new_dict["ptr"]])
        new_dict["know"] = 0
        new_dict["flag"] = 0
    ...
    for i in range(length):
        for j in range(i + 1, length):
            if new_list[i] < new_list[j]:
                reward += 1/(length ** 2 - length)
            if old_list[i] < old_list[j]:
                baseline += 1/(length ** 2 - length)
